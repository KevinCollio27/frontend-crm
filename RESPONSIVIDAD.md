...
- [x] Configuración > Equipo > Invitados (`InvitedUsersTable.tsx`) — toolbar (1 filtro fijo — "Estado", ancho completo en mobile, mismo criterio que Campañas/Blog) + Reset como botón normal (no hace falta grid, es 1 solo filtro), tabla, reusa `InviteUserSheet` ya arreglado (no hay sheet propio)
