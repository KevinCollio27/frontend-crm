...
import type { ActivityDetailStatus, DealDetail, HistorialType } from "../../data"

// ─── Configs ──────────────────────────────────────────────────────────────────

const HISTORIAL_ICON: Record<HistorialType, { Icon: LucideIcon; bg: string; icon: string }> = {
  created:      { Icon: PlusCircleIcon, bg: "bg-emerald-100", icon: "text-emerald-600" },
  stage_change: { Icon: ArrowRightIcon, bg: "bg-violet-100",  icon: "text-violet-600"  },
  note:         { Icon: FileTextIcon,   bg: "bg-amber-100",   icon: "text-amber-600"   },
  email:        { Icon: MailIcon,       bg: "bg-sky-100",     icon: "text-sky-600"     },
  activity:     { Icon: CalendarIcon,   bg: "bg-indigo-100",  icon: "text-indigo-600"  },
  quotation:    { Icon: ClipboardIcon,  bg: "bg-teal-100",    icon: "text-teal-600"    },
  responsible:  { Icon: UserIcon,       bg: "bg-orange-100",  icon: "text-orange-600"  },
}

const ACTIVITY_STATUS: Record<ActivityDetailStatus, { label: string; className: string }> = {
...
export function HistorialTab({ deal }: Props) {
  const upcoming = deal.activities.filter(
    (a) => a.status === "pendiente" || a.status === "en_progreso"
  )

  const months: string[] = []
  const grouped: Record<string, typeof deal.historial> = {}
  for (const entry of deal.historial) {
