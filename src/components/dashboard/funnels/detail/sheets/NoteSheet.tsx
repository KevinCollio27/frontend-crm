...
interface Props {
  ...
  note?: NoteRaw
  onSuccess?: (result: NoteRaw) => void
}

export function NoteSheet({ open, onOpenChange, opportunityId, note, onSuccess }: Props) {
  const isEdit = !!note
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      let result: NoteRaw
      if (isEdit && note) {
        console.log(`[notes-opp] update start id=${note.id}`)
        result = await noteService.update(note.id, { title: title.trim(), content })
        console.log(`[notes-opp] update OK → ${Date.now() - t0}ms`)
        notify.info({ title: "Nota actualizada", description: `"${title.trim()}" fue actualizada.` })
      } else {
        console.log(`[notes-opp] create start opportunityId=${opportunityId}`)
        result = await noteService.create({ title: title.trim(), content, opportunity_id: opportunityId })
        console.log(`[notes-opp] create OK → ${Date.now() - t0}ms`)
        ...
        <SheetHeader className="border-b px-5 py-4">
          ...
          <SheetDescription>
            {isEdit
              ...
              : "Agrega una nota sobre esta oportunidad."}
          </SheetDescription>
        </SheetHeader>
