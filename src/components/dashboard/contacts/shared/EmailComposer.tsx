...
export interface EmailComposerProps {
  connectionId:  number
  userEmail:     string
  // Un contacto puede tener más de un correo cargado — se precargan todos como
  // chips removibles, así el usuario decide si manda a uno, a varios o los quita.
  contactEmails: string[]
  contactName:   string | null
  // Si el envío parte de una oportunidad, el backend ya arma solo el log
  // ("El usuario envió un correo a...") y la fila de seguimiento en
  // opportunity_email — no hace falta nada más acá que pasar el id.
  opportunityId?: number
  onSent?:       () => void
}

export function EmailComposer({ connectionId, userEmail, contactEmails, contactName, opportunityId, onSent }: EmailComposerProps) {
  const [recipients, setRecipients]         = React.useState<string[]>([...new Set(contactEmails)])
  ...
  return (
    <div className="overflow-hidden rounded-xl border bg-card shadow-sm">
      ...
      {signatureHtml && (
        <Collapsible open={signatureOpen} onOpenChange={setSignatureOpen} className="border-t">
          <CollapsibleTrigger className="flex w-full items-center justify-between gap-2 px-3.5 py-2 text-left">
            <span className="text-xs text-muted-foreground">Firma (se agrega automáticamente al enviar)</span>
            <ChevronDownIcon className={cn("size-3.5 shrink-0 text-muted-foreground transition-transform", signatureOpen && "rotate-180")} />
          </CollapsibleTrigger>
...
export function NoGmailConnected() {
  return (
