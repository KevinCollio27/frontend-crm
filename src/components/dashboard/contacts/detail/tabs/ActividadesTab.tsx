...
import type { ContactActivity, ContactActivityStatus, ContactDetail } from "../../data"

// ─── Configs ──────────────────────────────────────────────────────────────────

const STATUS_CONFIG: Record<ContactActivityStatus, { label: string; className: string }> = {
...
function getColumns(): ColumnDef<ContactActivity>[] {
  return [
    {
      id: "select",
      header: ({ table }) => (
    ...
    },
    {
      ...
      header: "Estado",
      cell: ({ row }) => {
        const conf = STATUS_CONFIG[row.getValue("status") as ContactActivityStatus]
        return (
...
export function ActividadesTab({ contact }: Props) {
