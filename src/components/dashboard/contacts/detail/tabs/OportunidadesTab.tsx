...
import type { ContactDetail, ContactOpportunity, ContactOpportunityStatus } from "../../data"

// ─── Configs ──────────────────────────────────────────────────────────────────

const STATUS_CONFIG: Record<ContactOpportunityStatus, { label: string; className: string }> = {
...
function getColumns(): ColumnDef<ContactOpportunity>[] {
  return [
    {
      id: "select",
      header: ({ table }) => (
    ...
    },
    {
      ...
      header: "Estado",
      cell: ({ row }) => {
        const conf = STATUS_CONFIG[row.getValue("status") as ContactOpportunityStatus]
        return (
...
export function OportunidadesTab({ contact }: Props) {
