...
function contextBadge(note: NoteRaw): { label: string; name: string } | null {
  if (note.opportunity?.name)            return { label: "Oportunidad ligada", name: note.opportunity.name }
  if (note.opportunity_activity?.title)  return { label: "Actividad ligada",   name: note.opportunity_activity.title ?? "" }
  if (note.person_interest?.description) return { label: "Interés ligado",     name: note.person_interest.description }
  return null
}
...
export function NotasTab({ contactId }: Props) {
  ...
  React.useEffect(() => {
    ...
    noteService
      .list({ person_id: contactId, take: PAGE_SIZE, page: 1 })
      .then((res) => {
        if (cancelled) return
        setNotes(res.data)
  ...
  function loadMore() {
    ...
    setLoadingMore(true)
    noteService
      .list({ person_id: contactId, take: PAGE_SIZE, page: nextPage })
      .then((res) => {
  ...
  async function handleDelete(note: NoteRaw) {
    ...
    try {
      const t0 = Date.now()
      console.log(`[notes-contact] delete start id=${note.id}`)
      await noteService.delete(note.id)
      console.log(`[notes-contact] delete OK → ${Date.now() - t0}ms`)
