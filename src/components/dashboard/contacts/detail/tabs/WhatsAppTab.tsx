...
import type { ContactDetail, ContactWspMessageStatus } from "../../data"

// ─── Configs ──────────────────────────────────────────────────────────────────

const STATUS_CONFIG: Record<ContactWspMessageStatus, { label: string; className: string }> = {
...
export function WhatsAppTab({ contact }: Props) {
  return (
    <div className="flex flex-col gap-4 p-4">

      <WspComposer contactPhone={contact.phone ?? ""} />
      ...
      {contact.whatsappMessages.length > 0 && (
        <div className="flex flex-col gap-3">
          ...
          {contact.whatsappMessages.map((msg) => {
