...
import { AlertTriangleIcon, CheckCircle2Icon, CircleIcon, GitMergeIcon, Loader2Icon, PlusIcon, SearchIcon, StarIcon, UsersIcon, XCircleIcon, XIcon } from "lucide-react"
...
import type { Person, PersonDuplicateCandidate, PersonDuplicateGroup } from "@/types/contact"
import {
  computeDetailPlan,
  computeFieldPlan,
  computeImpact,
  suggestSurvivor,
} from "./contact-merge-utils"
...
interface Props {
  open: boolean
  group: PersonDuplicateGroup
  onOpenChange: (open: boolean) => void
  onMerged: () => void
}

export function MergeContactsSheet({ open, group, onOpenChange, onMerged }: Props) {
  const [step, setStep] = React.useState(1)
  const [candidates, setCandidates] = React.useState<PersonDuplicateCandidate[]>(group.candidates)
  const [extraIds, setExtraIds] = React.useState<Set<number>>(new Set())
  const [survivorId, setSurvivorId] = React.useState<number | null>(
    group.candidates.length > 0 ? suggestSurvivor(group.candidates) : null
  )
  ...
  React.useEffect(() => {
    ...
    if (survivorId === null || !candidates.some((p) => p.id === survivorId)) {
      setSurvivorId(suggestedId)
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [candidates, suggestedId])

  const survivor = candidates.find((p) => p.id === survivorId)
  const losers = candidates.filter((p) => p.id !== survivorId)
  ...
  function handleAddCandidate(person: PersonDuplicateCandidate) {
    setCandidates((prev) => [...prev, person])
    setExtraIds((prev) => new Set(prev).add(person.id))
  }
  ...
  async function handleConfirm() {
    if (!survivor) return
    const confirmed = await contactConfirm.merge(candidates.length, survivor.name)
    if (!confirmed) return
    ...
    try {
      ...
      notify.success({
        ...
        description: `${candidates.length} contactos se fusionaron en "${survivor.name}".`,
      })
      onMerged()
    } catch {
      notify.error({ title: "Algo salió mal", description: "No se pudo completar la fusión." })
    } finally {
      setSubmitting(false)
    }
  }
        ...
        <div className="flex items-start justify-between gap-4 border-b px-5 py-4">
          <div className="space-y-2">
            <SheetTitle className="flex items-center gap-2">
              ...
              {group.candidates.length > 0 ? <>Fusionar: &quot;{group.candidates[0].name}&quot;</> : "Fusión manual"}
            </SheetTitle>
            <SheetDescription>
              {candidates.length} contactos en esta fusión
              {extraIds.size > 0 && group.candidates.length > 0 && ` (${group.candidates.length} detectados + ${extraIds.size} agregados)`}
            </SheetDescription>
            {group.hasMultipleOrgs && (
              <p className="flex items-center gap-1.5 text-xs text-amber-700 dark:text-amber-400">
                <AlertTriangleIcon className="size-3.5 shrink-0" />
                Estos contactos pertenecen a organizaciones distintas — confirma que es la misma persona antes de fusionar.
              </p>
            )}
...
function Step1ChooseSurvivor({
  ...
  onAdd: (person: PersonDuplicateCandidate) => void
    ...
    <div className="space-y-5">
      <div className="space-y-3">
        <p className="text-sm text-muted-foreground">
          {candidates.length > 0
            ? "Elige cuál contacto queda como registro base. Los datos de los demás se usarán para completarlo."
            : "Busca y agrega al menos 2 contactos para empezar la fusión."}
        </p>
        {candidates.map((person) => {
          const selected = person.id === survivorId
          const isExtra = extraIds.has(person.id)
          return (
            <div
              key={person.id}
              className={cn(
            ...
            >
              <button type="button" onClick={() => onSelect(person.id)} className="flex flex-1 items-center gap-3 min-w-0 text-left">
                ...
                <div className="min-w-0">
                  ...
                  <p className="truncate text-xs text-muted-foreground">
                    #{person.id} · {person.organization_name ?? "Sin organización"} · creado {format(new Date(person.created_at), "d MMM, HH:mm", { locale: es })}
                  </p>
                  <p className="text-xs text-muted-foreground">
                    {person.counts.opportunities} vacantes · {person.counts.notes} notas · {person.counts.interests} intereses
                  </p>
                </div>
              </button>
              <div className="flex shrink-0 items-center gap-2">
                {person.id === suggestedId && (
      ...
      <AddContactSearch excludeIds={candidates.map((p) => p.id)} onAdd={onAdd} />
    </div>
  )
}
...
function AddContactSearch({
  ...
  React.useEffect(() => {
    ...
    const t = setTimeout(() => {
      contactService
        ...
        .then((res) => { if (!cancelled) setResults(res.data.filter((p) => !excludeIds.includes(p.id))) })
  ...
  async function handlePick(person: Person) {
    ...
    try {
      const candidate = await contactService.getMergeCandidate(person.id)
      onAdd(candidate)
      ...
      notify.error({ title: "Algo salió mal", description: "No se pudo agregar el contacto." })
    } finally {
      setAddingId(null)
    }
  }
  ...
  return (
    <div className="space-y-2 border-t pt-4">
      <p className="text-xs font-medium text-muted-foreground">¿Falta un contacto? Búscalo y agrégalo a esta fusión.</p>
          ...
          {searching ? (
            ...
            <div className="divide-y">
              {results.map((person) => (
                ...
                >
                  <span className="truncate">{person.name}{person.organization ? ` · ${person.organization.name}` : ""}</span>
                  {addingId === person.id ? <Loader2Icon className="size-3.5 shrink-0 animate-spin" /> : <PlusIcon className="size-3.5 shrink-0 text-muted-foreground" />}
                </button>
              ))}
            </div>
          )}
  ...
  return (
    <div className="space-y-5">
            ...
            {fieldPlan.map((row) => (
              <tr key={row.field} className="align-top">
                ...
                <td className="px-3 py-2.5">
                  {row.status === "empty" ? (
                    <span className="text-muted-foreground">(vacío en todos)</span>
                  ) : row.status === "conflict" ? (
                ...
                <td className="px-3 py-2.5 text-xs text-muted-foreground">
                  ...
                  {row.status === "auto" && row.source !== "Base" && " (único con dato)"}
                </td>
              </tr>
            ))}
            {detailPlan.map((row) => (
              <tr key={row.key} className="align-top">
                ...
                <td className="px-3 py-2.5">
                  {row.status === "conflict" ? (
                    <ChoiceRow
                      options={row.options!.map((o) => ({ ...o, label: o.value }))}
                      selected={detailChoices[row.key] ?? row.value}
                      onChoose={(v) => onDetailChoice(row.key, v)}
                    />
                  ) : (
                    row.value
                  )}
      ...
      <div className="rounded-lg border bg-muted/30 p-4 text-sm">
        <p className="mb-2 font-medium">Al confirmar se reasignan al contacto final:</p>
        <ul className="grid grid-cols-2 gap-1 text-muted-foreground">
          ...
          <li>• {impact.interests} intereses</li>
        </ul>
        <p className="mt-3 text-xs text-muted-foreground">
          Los contactos no elegidos quedan archivados — nunca se borran. Queda respaldo completo por si algo sale mal.
        </p>
      </div>
    </div>
  )
