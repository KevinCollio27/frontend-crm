...
export function prefetchHistorial(orgId: number): void {
  if (_pending.has(orgId) || _resolved.has(orgId)) return
  const p = systemLogService
    .list({ page: 1, take: PAGE_SIZE, keyEntity: "organization", keyEntityId: orgId })
    .then((res) => {
      _resolved.set(orgId, { logs: res.data, hasMore: res.nextPage !== null })
    })
    .catch(() => {})
  _pending.set(orgId, p)
}
...
export function HistorialTab({ orgId }: Props) {
  const [logs, setLogs]               = React.useState<SystemLogRaw[]>(() => _resolved.get(orgId)?.logs ?? [])
  const [hasMore, setHasMore]         = React.useState<boolean>(() => _resolved.get(orgId)?.hasMore ?? false)
  const [page, setPage]               = React.useState(1)
  const [loading, setLoading]         = React.useState(() => !_resolved.has(orgId))
  ...
  // nota, vincular un contacto, etc.) — escucha todas las entidades que loguean
  // contra esta organización y refresca cuando corresponde a este orgId.
  useEntityRealtime("organization", (payload) => {
    if ((payload.data as { id?: number })?.id === orgId) setRefreshKey((k) => k + 1)
  })
  useEntityRealtime("note", (payload) => {
    if ((payload.data as { organization_id?: number | null })?.organization_id === orgId) setRefreshKey((k) => k + 1)
  })
  useEntityRealtime("organization-challenge", (payload) => {
    if ((payload.data as { organization_id?: number })?.organization_id === orgId) setRefreshKey((k) => k + 1)
  })
  useEntityRealtime("contact", (payload) => {
    if (payload.action === "deleted" || (payload.data as { organization_id?: number | null })?.organization_id === orgId) {
      setRefreshKey((k) => k + 1)
    }
  })
  ...
  React.useEffect(() => {
    const cached  = _resolved.get(orgId)
    const pending = _pending.get(orgId)
    _resolved.delete(orgId)
    _pending.delete(orgId)
    ...
    if (pending) {
      pending.then(() => {
        if (cancelled) return
        const data = _resolved.get(orgId)
        _resolved.delete(orgId)
        if (!data) return
        setLogs(data.logs)
  ...
  function loadMore() {
    ...
    setLoadingMore(true)
    systemLogService
      .list({ page: nextPage, take: PAGE_SIZE, keyEntity: "organization", keyEntityId: orgId })
      .then((res) => {
  ...
  if (!logs.length) {
    return (
      <p className="py-12 text-center text-sm text-muted-foreground">
        Sin actividad registrada para esta organización.
      </p>
    )
  }
