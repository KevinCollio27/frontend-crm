...
import type { OrganizationActivityStatus, OrganizationDetail, OrganizationHistorialType } from "../../data"

// ─── Configs ──────────────────────────────────────────────────────────────────

const HISTORIAL_ICON: Record<OrganizationHistorialType, { Icon: LucideIcon; bg: string; icon: string }> = {
  ...
  challenge:     { Icon: StarIcon,       bg: "bg-rose-100",    icon: "text-rose-600"    },
  contact_added: { Icon: ArrowRightIcon, bg: "bg-teal-100",    icon: "text-teal-600"    },
}

const ACTIVITY_STATUS: Record<OrganizationActivityStatus, { label: string; className: string }> = {
...
export function HistorialTab({ organization }: Props) {
  const upcoming = organization.activities.filter(
    (a) => a.status === "pendiente" || a.status === "en_progreso"
  )

  const months: string[] = []
  const grouped: Record<string, typeof organization.historial> = {}
  for (const entry of organization.historial) {
