...
function contextLabel(note: NoteRaw): string | null {
  if (note.opportunity?.name)           return note.opportunity.name
  if (note.opportunity_activity?.title) return note.opportunity_activity.title
  return null
}
...
export function NotasTab({ orgId }: Props) {
  ...
  React.useEffect(() => {
    ...
    noteService
      .list({ organization_id: orgId, take: PAGE_SIZE, page: 1 })
      .then((res) => {
        if (cancelled) return
        setNotes(res.data)
  ...
  function loadMore() {
    ...
    setLoadingMore(true)
    noteService
      .list({ organization_id: orgId, take: PAGE_SIZE, page: nextPage })
      .then((res) => {
              ...
              return (
                <div key={note.id} className="flex min-w-0 gap-3.5 px-4 py-4">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    />
                    {ctx && (
                      <span className="mt-2 inline-flex items-center rounded-md border bg-muted px-2 py-0.5 text-xs text-muted-foreground">
                        {ctx}
                      </span>
                    )}
                  </div>
