...
import type { OrganizationActivity, OrganizationActivityStatus, OrganizationDetail } from "../../data"

// ─── Configs ──────────────────────────────────────────────────────────────────

const STATUS_CONFIG: Record<OrganizationActivityStatus, { label: string; className: string }> = {
...
function getColumns(): ColumnDef<OrganizationActivity>[] {
  return [
    {
      id: "select",
      header: ({ table }) => (
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="text-sm text-muted-foreground">{row.getValue("opportunity_name")}</div>
      ),
    },
    {
    ...
    },
    {
      ...
      header: "Estado",
      cell: ({ row }) => {
        const conf = STATUS_CONFIG[row.getValue("status") as OrganizationActivityStatus]
        return (
...
export function ActividadesTab({ organization }: Props) {
