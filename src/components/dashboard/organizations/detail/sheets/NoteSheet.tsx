...
export function NoteSheet({ open, onOpenChange, orgId, note, onSuccess }: Props) {
  const isEdit = !!note
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (isEdit && note) {
        ...
        await noteService.create({ title: title.trim(), content, organization_id: orgId })
        ...
        <SheetHeader className="border-b px-5 py-4">
          ...
          <SheetDescription>
            {isEdit
              ...
              : "Agrega una nota sobre esta organización."}
          </SheetDescription>
        </SheetHeader>
