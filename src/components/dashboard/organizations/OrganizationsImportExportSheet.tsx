...
import { organizationService, type ImportPreviewError, type ImportPreviewRow } from "@/services/organization.service"
...
const EXPORT_FILE_PREFIX = "organization_"
...
// El RUT manda sobre el nombre: si dos filas comparten RUT son la misma
// organización aunque el nombre varíe; si no hay RUT, el nombre es lo único
// que tenemos para detectar duplicados.
function computeRowFlags(
  ...
  for (const row of rows) {
    const docNorm = row.document_number ? row.document_number.trim() : null
    const nameNorm = row.name ? normalize(row.name) : null
    let reason: string | null = null

    if (docNorm && existingDocs.has(docNorm)) reason = "RUT ya existe"
    else if (!docNorm && nameNorm && existingNames.has(nameNorm)) reason = "Nombre ya existe"
    else if (docNorm && seenDocs.has(docNorm)) reason = "RUT repetido en el archivo"
    else if (!docNorm && nameNorm && seenNames.has(nameNorm)) reason = "Nombre repetido en el archivo"

    if (reason) flags.set(row.row, reason)
    if (docNorm) seenDocs.add(docNorm)
...
export function OrganizationsImportExportSheet({ open, onOpenChange, totalOrganizations, onImported }: Props) {
  ...
  async function handleExport() {
    ...
    setExportError("")
    try {
      await organizationService.exportOrganizations()
      setStep("export-generating")
    } catch {
  ...
  const [existingNameSet, setExistingNameSet] = React.useState<Set<string>>(new Set())
  const [existingDocSet, setExistingDocSet] = React.useState<Set<string>>(new Set())
  ...
  const rowFlags = React.useMemo(
    () => computeRowFlags(previewRows, existingNameSet, existingDocSet),
    [previewRows, existingNameSet, existingDocSet]
  )
  ...
  async function handleDownloadTemplate() {
    ...
    try {
      const url = await organizationService.getImportTemplate()
      const a = document.createElement("a")
      a.href = url
      a.download = "plantilla_organizaciones.xlsx"
      a.click()
    } catch {
  ...
  async function handleUploadContinue() {
    ...
    try {
      ...
      const preview = await organizationService.previewImport(base64)

      const names = preview.rows.map((r) => r.name)
      const docs = preview.rows.filter((r) => r.document_number).map((r) => r.document_number as string)
      const dup = await organizationService.checkDuplicates(names, docs).catch(() => ({ existingNames: [], existingDocumentNumbers: [] }))

      const nameSet = new Set(dup.existingNames.map(normalize))
      const docSet = new Set(dup.existingDocumentNumbers.map((d) => d.trim()))
      const flags = computeRowFlags(preview.rows, nameSet, docSet)

      setPreviewRows(preview.rows)
      setPreviewErrors(preview.errors)
      setExistingNameSet(nameSet)
      setExistingDocSet(docSet)
  ...
  async function handleConfirmImport() {
    ...
    const results = await Promise.allSettled(
      toImport.map((row) => {
        const payload: OrganizationPayload = {
          ...
          pais_origen: "CL",
          address: row.address && row.address_label_id
            ? [{ id: null, label_id: row.address_label_id, option: row.address_option ?? "", value: row.address }]
            : [],
          tag: [],
        }
    ...
    results.forEach((r, i) => {
      ...
      else failed.push({ row: toImport[i].row, name: toImport[i].name, reason: errorMessage(r.reason, "Error al crear la organización") })
    })
  ...
  const TITLES: Record<Step, string> = {
    "choose": "Importar / Exportar organizaciones",
    "export-confirm": "Exportar organizaciones",
    "export-generating": "Exportar organizaciones",
    "export-ready": "Exportar organizaciones",
    "export-error": "Exportar organizaciones",
    "import-upload": "Importar organizaciones",
    "import-review": "Revisar antes de importar",
    "import-result": "Importación completa",
  }

  const DESCRIPTIONS: Record<Step, string> = {
    "choose": "Gestiona las organizaciones de tu espacio de trabajo",
    "export-confirm": "Se genera un archivo Excel con todas tus organizaciones",
    ...
    "import-result": `${importSucceeded} organización${importSucceeded !== 1 ? "es" : ""} importada${importSucceeded !== 1 ? "s" : ""}`,
  }
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent
        ...
        style={{ maxWidth: step === "import-review" ? 760 : 480, padding: 0, gap: 0 }}
        ...
        <div className="flex-1 space-y-3 overflow-y-auto p-5">
            ...
            <>
              ...
              >
                ...
                <div className="min-w-0 flex-1 space-y-1">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Importa organizaciones a tu espacio de trabajo desde una plantilla.
                  </p>
                </div>
              ...
              >
                ...
                <div className="min-w-0 flex-1 space-y-1">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Exporta las organizaciones de tu espacio de trabajo en Excel.
                  </p>
                </div>
                <ChevronRightIcon className="mt-0.5 size-4 shrink-0 text-muted-foreground" />
              </button>
          ...
          {step === "export-confirm" && (
            <div className="rounded-lg border border-teal-200 bg-teal-50 p-3 dark:border-teal-900 dark:bg-teal-950/20">
              ...
              <p className="text-sm">
                Se exportarán las {totalOrganizations.toLocaleString("es-CL")} organizaciones de tu espacio de
                trabajo con su información completa (RUT, dirección, industria, redes sociales, etiquetas y usuario
                asignado) en un archivo Excel.
              </p>
            </div>
          )}
          ...
          {step === "export-ready" && (
            <div className="flex flex-col items-center justify-center gap-3 py-16 text-center">
              ...
              <div className="space-y-0.5">
                ...
                <p className="text-xs text-muted-foreground">{totalOrganizations.toLocaleString("es-CL")} organizaciones</p>
              </div>
            </div>
          )}
          ...
          {step === "import-upload" && (
            <>
              <div className="rounded-lg border border-teal-200 bg-teal-50 p-3 dark:border-teal-900 dark:bg-teal-950/20">
                ...
                <p className="text-sm">
                  Usa nuestra plantilla oficial para cargar organizaciones masivamente. Descárgala, complétala y
                  súbela aquí mismo. Antes de importar podrás revisar los datos y descartar duplicados.
                </p>
              </div>
                ...
                <Table className="min-w-160">
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead className="min-w-70">Organización</TableHead>
                      <TableHead className="min-w-32">RUT</TableHead>
                      <TableHead className="min-w-60">Dirección</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                          ...
                          <TableRow key={row.row}>
                            ...
                            <TableCell className="align-top whitespace-normal">
                              <Input
                                ...
                                onChange={(e) => updateRow(row.row, { document_number: e.target.value || null })}
                              />
                            </TableCell>
                            <TableCell className="w-60 align-top whitespace-normal">
                              <Input
                                ...
                                value={row.address ?? ""}
                                onChange={(e) => updateRow(row.row, { address: e.target.value || null })}
                              />
                            </TableCell>
                          </TableRow>
                        )
          ...
          {step === "import-result" && (
            <>
              <div className="flex flex-col items-center justify-center gap-3 py-12 text-center">
                <FileSpreadsheetIcon className="size-8 text-emerald-600 dark:text-emerald-500" />
                <p className="text-sm font-medium">
                  {importSucceeded} organización{importSucceeded !== 1 ? "es" : ""} importada{importSucceeded !== 1 ? "s" : ""}
                </p>
