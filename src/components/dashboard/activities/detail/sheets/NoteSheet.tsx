...
export function NoteSheet({ open, onOpenChange, activityId, note, onSuccess }: Props) {
  const isEdit = !!note
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (isEdit && note) {
        ...
        await noteService.create({ title: title.trim(), content, opportunity_activity_id: activityId })
        notify.success({ title: "Nota creada", description: `"${title.trim()}" se creó correctamente.` })
      }
      onSuccess?.()
      onOpenChange(false)
    } catch {
      notify.error({ title: "No se pudo guardar la nota", description: "Intenta de nuevo." })
    } finally {
      setSubmitting(false)
    }
  }
        ...
        <SheetHeader className="border-b px-5 py-4">
          ...
          <SheetDescription>
            {isEdit
              ...
              : "Agrega una nota sobre esta actividad."}
          </SheetDescription>
        </SheetHeader>
