...
// Página que recibe la vuelta del popup del Embedded Signup de Meta (a diferencia del
// callback de Gmail, acá el frontend arma la URL de OAuth y también intercambia el code —
// ver WhatsAppIntegrationSheet.tsx). Es un simple relay: no decide nada, solo avisa a la
// ventana que la abrió (incluyendo el caso "hay que elegir número") y se cierra.
export default function WhatsAppCallbackPage() {
  ...
  React.useEffect(() => {
    ...
    if (error) {
      setStatus("error")
      setMessage(error)
      window.opener?.postMessage({ type: "WHATSAPP_AUTH_ERROR", message: error }, window.location.origin)
    ...
    if (!code) {
      ...
      window.opener?.postMessage({ type: "WHATSAPP_AUTH_ERROR", message: "No llegó ningún code de Meta." }, window.location.origin)
      setTimeout(() => window.close(), 1500)
      return
    }
    ...
    whatsappService
      .completeEmbeddedSignup(code, redirectUri)
      .then((result) => {
        setStatus("success")
        setMessage("WhatsApp conectado correctamente.")
        window.opener?.postMessage({ type: "WHATSAPP_AUTH_SUCCESS", ...result }, window.location.origin)
        window.close()
      })
      .catch((err: { message?: string }) => {
        ...
        setStatus("error")
        setMessage(msg)
        window.opener?.postMessage({ type: "WHATSAPP_AUTH_ERROR", message: msg }, window.location.origin)
        setTimeout(() => window.close(), 1500)
      })
  }, [])
