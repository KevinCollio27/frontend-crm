...
// integration.controller.ts → handleGoogleContactsCallback). Solo avisa a la ventana que la
// abrió y se cierra sola — no forma parte de la navegación normal del CRM.
export default function GoogleContactsCallbackPage() {
  ...
  React.useEffect(() => {
    ...
    if (errorCode) {
      const errorMessage = ERROR_MESSAGES[errorCode] ?? "No se pudo conectar Google Contacts."
      setStatus("error")
      setMessage(errorMessage)
      window.opener?.postMessage(
        { type: "GOOGLE_CONTACTS_AUTH_ERROR", message: errorMessage },
        window.location.origin
      )
    } else {
      setStatus("success")
      setMessage("Google Contacts conectado correctamente.")
      window.opener?.postMessage({ type: "GOOGLE_CONTACTS_AUTH_SUCCESS" }, window.location.origin)
    }
