...
// integration.controller.ts → handleGoogleCallback). Solo avisa a la ventana que la abrió
...
export default function GoogleCalendarCallbackPage() {
  ...
  React.useEffect(() => {
    ...
    if (errorCode) {
      const errorMessage = ERROR_MESSAGES[errorCode] ?? "No se pudo conectar Google Calendar."
      setStatus("error")
      setMessage(errorMessage)
      window.opener?.postMessage(
        { type: "GOOGLE_CALENDAR_AUTH_ERROR", message: errorMessage },
        window.location.origin
      )
    } else {
      setStatus("success")
      setMessage("Google Calendar conectado correctamente.")
      window.opener?.postMessage({ type: "GOOGLE_CALENDAR_AUTH_SUCCESS" }, window.location.origin)
    }
